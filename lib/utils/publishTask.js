...
function buildDockerImage(packageJson) {
  return Promise.coroutine(function* () {
...
exports.run = () => {
  Promise.coroutine(function* () {
    try {
      if (!workingDir || (!buildTask || !dockerProject) || (!dockerProject && !npmProject && !publishTask)) {
        commands.logError('Working dir, build task and publish task must be defined.');
        process.exit(1);
      }
      ...
      if (dockerProject) {
        yield buildDockerImage(packageJson);
      }
      if (beforePublishTask) {
        yield beforePublishTask();
      }
      if (dockerProject) {
