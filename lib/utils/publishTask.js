...
function buildImage(packageJson) {
  return Promise.coroutine(function* () {
    ...
    yield exec(`docker build -t ${ECR_REPOSITORY_NAMESPACE}/${packageJson.name} .`);
    commands.log('Tagging image...');
    yield exec(`docker tag ${ECR_REPOSITORY_NAMESPACE}/${packageJson.name}:latest ${ECR_REPOSITORY_URL}/${ECR_REPOSITORY_NAMESPACE}/${packageJson.name}:latest`);
  })();
}
